...
def get_pytorch_obj(trail:optuna.Trial):
    model = model_wizard(
...
""" Save to file """
def save2file(model, params:optuna.Trial.params):
    # Save the result
    with open(args.root+os.sep+'param_opt.csv', 'w', newline='') as csvfile:
...
def objective(trial:optuna.Trial):
    # Get pytorch objs
    model, optimizer, lr_sch = get_pytorch_obj(trial)
    # Get datasets
    ...
    for epo in range(config['NUM_EPOCH']):
        ...
        trial.report(acc_t1, epo)
        if trial.should_prune():
            raise optuna.exceptions.TrialPruned()
    # Store the model in the trial's user attributes
    trial.set_user_attr(key="trained_model", value=model)
    return acc_t1

""" Main function """
def main():
    study = optuna.create_study(direction="maximize")
    study.optimize(objective, n_trials=100, timeout=600, n_jobs=config['N_JOB'])
    best = study.best_trial
    model = best.user_attrs["trained_model"]
    params = best.params
